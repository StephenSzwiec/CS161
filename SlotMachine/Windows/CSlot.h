...
#include "CMoney.h"
#include "CSpinner.h"
#include <windows.h> //character buffer wizardry
...
public:
	...
	void displayClear() {
		//cout << "\033c" << endl; //always works for POSIX terminals
		//cout << "\x1B[2J\x1B[H" << endl; //does not always work for windows terminals
	}

	//utility function to clear the terminal (windows port)
	void windowsClear(HANDLE hConsole) {
		...
		if (!FillConsoleOutputCharacter(hConsole, (TCHAR) ' ', dwConSize, coordScreen, &cCharsWritten)) //fill the entire buffer with spaces
		{
			return;
		}
		...
		//reset the standard output cursor to the top left 
	...
	void run() {
		HANDLE currentTerminal = GetStdHandle(STD_OUTPUT_HANDLE); //windows.h method used to handle current terminal window
		...
		windowsClear(currentTerminal); //clear the terminal before we display menu
